function contour_demo(demo_id,im)
...
%             im=imread('./nematodes.png');
%             im=im(:,:,1);            
            mask=line_detector(im,alfa1,thr);
        ...
        case 5%leaf
            alfa1=0.8; alfa2=0.8; thr=0.4;
            im=imread('./leaf.png');
%             im=im(:,:,1);            
            mask=edge_detector(im,alfa1,alfa2,thr);
         case 6%cell
            alfa1=0.6; alfa2=0.9; thr=0.2;
            im=imread('./cell4.tif');
%             im=im(:,:,1);            
            mask=edge_detector(im,alfa1,alfa2,thr);
         case 7%plantcell
            alfa1=0.6; alfa2=0.8; thr=0.25;
            im=imread('./arab.jpg');
            im=im(:,:,1);            
            mask=edge_detector(im,alfa1,alfa2,thr);
         case 8 %probability pavement cell line 
            alfa1=0.95; thr=0.20;
            im=imread('./prob_ime.png');
%             im=im(:,:,1);            
            mask=line_detector(im,alfa1,thr);
         case 9 %probability pavement cell line 
            alfa1=0.95; thr=0.15;
            im=255-imread('./prob_imp.png');
%             im=im(:,:,1);            
            mask=line_detector(im,alfa1,thr);
    end
    mask=imdilate(mask,strel('disk',1));
    figure,imshow(drawmask(im,mask));
 end
