%% Reconstrueert een ruizig beeld a.d.h.v. het BM3D algoritme
function sig_est = fBM3D(sig, sigma)
%% 

%% Extra parameters
profile = 'np'; % 'np' (normal profile) of 'lc' (fast profile)
print_to_screen = 0;
%% 

[~, sig_est] = BM3D(1, uint8(sig), sigma, profile, print_to_screen);
sig_est = sig_est.*255;
